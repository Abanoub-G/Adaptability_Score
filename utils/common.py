...
class logs():
    ...
    def append(self, noise_type, N_T, accuracy):
    ...
    def write_file(self, file_name):
        ...
        with open(file_path, 'w') as log_file: 
            log_file.write('noise_type,N_T,A_T\n')
            for i in range(len(self.noise_type_array)):
                log_file.write('%s, %d, %3.6f\n' %\
                    (self.noise_type_array[i], self.N_T_array[i],self.accuracy_array[i]))
